...
# --- Load env from .env.example for Testing ---
# This ensures tests pass even if .env is missing (e.g., CI/CD)
...
def mock_health_condition_service():
    with patch(
        "app.routers.user_health_condition.HealthConditionService", autospec=True
    ) as mock:
        yield mock
...
def mock_profile_form_service():
    with patch(
        "app.routers.user_profile_form.ProfileFormService", autospec=True
    ) as mock:
        yield mock
