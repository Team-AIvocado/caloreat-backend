...
async def request_ai_inference(image_bytes: bytes, image_id: str, content_type: str, ai_url: str) -> dict:
    """
...
async def get_or_create_dummy_users(session: AsyncSession, count: int = 5) -> list[int]:
    ...
    # 기존 더미 유저 확인
    result = await session.execute(text("SELECT id FROM users WHERE email LIKE 'mlops_tester_%@example.com'"))
...
async def process_inner_zip(inner_zip_path: str, max_images_per_zip: int, session: AsyncSession, user_ids: list[int], ai_url: str):
    """
    ...
    try:
        with zipfile.ZipFile(inner_zip_path, 'r') as z:
            ...
            image_files = [f for f in file_list if f.lower().endswith(('.jpg', '.jpeg', '.png')) and '__MACOSX' not in f]
            ...
            for file_path in image_files:
                ...
                print(f"   -> Processing {food_name}: {filename} (User: {current_user_id})")
                ...
                # 2. Real AI Inference
                ai_response = await request_ai_inference(file_bytes, image_uuid, content_type, ai_url)
...
async def seed_from_nested_zip(root_zip_path: str, max_images_total: int, ai_url: str, dry_run: bool = False):
    """
    ...
    async with AsyncSessionLocal() as session:
        ...
        with zipfile.ZipFile(root_zip_path, 'r') as root_z:
            ...
            for inner_zip_name in inner_zips:
                ...
                print(f"[Processing Category] {inner_zip_name} ...")
                
                extract_path = os.path.join(temp_dir, inner_zip_name)
                ...
                if not dry_run:
                    count = await process_inner_zip(extract_path, limit_per_zip, session, user_ids, ai_url)
                    total_processed += count
                else:
                    print(f"   [Dry Run] Extracted {inner_zip_name}, would process ~{limit_per_zip} images.")
...
if __name__ == "__main__":
    ...
    parser.add_argument("--ai-url", default="http://localhost:8001", help="AI Service Base URL") 
    ...
    asyncio.run(seed_from_nested_zip(args.zip_path, args.max_images, args.ai_url, args.dry_run))
